...
export class Query {
  ...
  getCombinations() {
    ...
    const helper = (
      requiredFields: QueryField[],
      optionalFields: QueryField[]
    ): QueryField[][] => {
      ...
      const subGroupsWithOptionalField = helper(
        [...queryGroupWithOptionalField],
        [...optionalFields]
      );
      const subGroupsWithoutOptionalField = helper(
        [...queryGroup],
        [...optionalFields]
      );
    ...
    let orderByHelper = (
      combined: OrderByQueryFieldIndex[],
      orderByGroupFieldsToCombine: QueryOrderByField[]
    ) => {
      ...
      for (const combo of combinations) {
        const comboResult = orderByHelper(
          [...combined, combo],
          [...orderByGroupFieldsToCombine]
        );
        res = [...res, ...comboResult];
      }
    ...
    const orderByCombinations = this.orderByFields.flatMap((orderByGroup) =>
      orderByHelper([], orderByGroup)
    );
    const fieldCombinations = helper(requiredFields, optionalFields).map(
      (queryFields) => {
    ...
    console.log(
      `Found: ${indexes.length} indexes after filtering out indexes that don't include all required fields`
    );

    /**
     * remove duplicate fields in indexes
     */
    indexes = indexes.filter((index) => {
      const fieldIds = new Set(index.map((indexField) => indexField.id));
      for (const field of index) {
    ...
    /**
     * remove overlapping index fields between where and order by fields
     * preferring the order by field index mode
     * e.g. a DESC where field on field "a.b" and an ASC order by on field "a.b"
     */
    indexes = indexes.map((index) => {
      const orderBy = index.filter((item) => item.isOrderByField);
      return index.filter((field) => {
        if (field.isOrderByField) {
          return true;
        }
        const hasEquivalentOrderByField = orderBy.find((orderByField) => {
          const samePath = orderByField.fieldPath === field.fieldPath;
          const isAscOrDes = (indexMode: FirestoreIndexMode) => {
            return (
              indexMode === FirestoreIndexMode.Ascending ||
              indexMode === FirestoreIndexMode.Descending
            );
          };

          return (
            isAscOrDes(orderByField.indexMode) &&
            samePath &&
            isAscOrDes(field.indexMode) &&
            orderByField.indexMode !== field.indexMode
          );
        });

        return !hasEquivalentOrderByField;
      });
    });

    console.log(
      `Found: ${indexes.length} indexes after filtering out indexes that include excluded fields`
    );
  ...
  optimizeIndexes(
    indexes: (WhereQueryFieldIndex | OrderByQueryFieldIndex)[][]
  ) {
    ...
    let mergedIndexes: Map<
      string,
      { field: WhereQueryFieldIndex; orderBy?: OrderByQueryFieldIndex }
    > = new Map();

    const getMergedIndexId = (
      index: WhereQueryFieldIndex,
      orderByField?: OrderByQueryFieldIndex
    ) => {
    ...
    for (const indexFields of indexes) {
      ...
      if (
        canMergeFields &&
        orderByFields.length <= MAX_ORDER_BY_FIELDS_TO_MERGE
      ) {
        const orderBy = orderByFields.pop();
        let fieldIndexes = [...indexFields].filter(
          (item) => !item.isOrderByField
        ) as WhereQueryFieldIndex[];
    ...
    if (optimized <= startedWith) {
      console.log(
        `Optimization resulted in ${
          startedWith - optimized
        } less indexes. Returning optimized indexes.`
      );
      return optimizedIndexes;
    } else {
      console.log(
        `Optimization resulted in ${
          optimized - startedWith
        } more indexes. Returning original indexes.`
      );
  ...
  toFirestoreIndexes(
    ...
    const toFirestoreIndexField = (
      field: WhereQueryFieldIndex | OrderByQueryFieldIndex
    ) => {
  ...
  removeDuplicates(firestoreIndexes: FirestoreIndex[]) {
    ...
    for (const firestoreIndex of firestoreIndexes) {
      ...
      // remove duplicates
      const uniqueFields = firestoreIndex.fields.reduce(
        (acc: FirestoreIndexField[], field) => {
    ...
    const uniqueIndexes = firestoreIndexes
      .filter((index) => {
    ...
    console.log(
      `Removed ${
        firestoreIndexes.length - uniqueIndexes.length
      } duplicate indexes`
    );
